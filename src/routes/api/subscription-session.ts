    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          const { userId, planName, interval, priceUsd } = body;

          if (!planName || typeof priceUsd !== "number" || priceUsd <= 0) {
            return new Response(
              JSON.stringify({
                error: "Invalid subscription request: planName and valid priceUsd are required.",
              }),
