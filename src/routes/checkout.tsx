...
import { CreditCard, Lock, ShieldCheck, AlertTriangle, Wallet } from "lucide-react";
...
function Checkout() {
        ...
        try {
           ...
           if (auth.currentUser) {
             ...
             auth.onAuthStateChanged((fUser) => {
               if (fUser && !values.email) {
                 setValues((v) => ({ ...v, email: fUser.email! }));
               }
             });
           }
        } catch(e) {}
      }
      if (u?.email && !values.email) setValues((v) => ({ ...v, email: u!.email! }));
    });
  ...
  const pay = async (e: React.FormEvent) => {
    ...
    if (!result.success || Object.keys(manualErrors).length > 0) {
      const eObj: Errors = { ...manualErrors };
      if (!result.success) {
    ...
    try {
      ...
      if (paymentMethod === "crypto") {
        await new Promise((r) => setTimeout(r, 1500)); // Simulate Web3 interaction
        await supabase
          .from("orders")
          .update({ status: "paid" })
          .eq("id", order.id);
          ...
          <Section title="Payment">
            <div className="flex gap-4 mb-4">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-lg border ${paymentMethod === 'stripe' ? 'border-gold bg-gold/10 text-gold' : 'border-gold/20 text-muted-foreground'}`}
              >
                <CreditCard className="h-4 w-4" /> {t("pay_stripe")}
              </button>
              <button
                type="button"
                onClick={() => setPaymentMethod("crypto")}
                className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-lg border ${paymentMethod === 'crypto' ? 'border-gold bg-gold/10 text-gold' : 'border-gold/20 text-muted-foreground'}`}
              >
                <Wallet className="h-4 w-4" /> {t("crypto_payment")}
              </button>
            </div>

            {paymentMethod === "stripe" ? (
              <div className="grid sm:grid-cols-[1fr_auto] gap-3 items-start animate-fade-up">
                <Field
                  label="Card number"
              ...
              <div className="p-6 border border-gold/20 rounded-lg text-center animate-fade-up bg-muted/20">
                <Wallet className="h-8 w-8 mx-auto text-gold mb-3" />
                <h4 className="font-display text-lg mb-2">Web3 Payment</h4>
                <p className="text-sm text-muted-foreground mb-4">Pay securely using Solana or Ethereum. A wallet prompt will appear upon submission.</p>
                <div className="text-xs px-3 py-1 bg-gold/10 text-gold inline-block rounded-full">
