...
function Dashboard() {
  ...
  const [notifications, setNotifications] = useState<NotificationItem[]>([
    ...
    },
    {
      ...
      description: "You are currently allocated to the Standard plan with basic concierge access.",
      time: "5 minutes ago",
      read: false,
      type: "success",
    },
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      if (!u) {
        ...
        try {
          ...
          if (auth.currentUser) {
            u = { id: auth.currentUser.uid, email: auth.currentUser.email } as any;
          } else {
            // Also listen to Firebase auth state
            auth.onAuthStateChanged((firebaseUser) => {
              if (firebaseUser) {
                const fUser = { id: firebaseUser.uid, email: firebaseUser.email } as any;
                setUser(fUser);
                if (fUser.email === "gulshanqudratova715@gmail.com") {
                  setUserRole("admin");
                } else {
                  setUserRole("member");
                }
      ...
      // Demo Mode Fallback
      if (!u && localStorage.getItem("demo_mode") === "true") {
        u = {
          id: "demo-user-123",
          email: "demo@luxemagnate.com",
    ...
    const { data: sub } = supabase.auth.onAuthStateChange(async (_e, session) => {
      ...
      if (!u) {
        try {
          const { auth } = await import("@/lib/firebase");
          if (auth.currentUser) {
             return; // Don't overwrite if firebase user exists
          }
        } catch (e) {}
      }
      setUser(u);
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.hasStripe && data.url) {
          toast.success("Redirecting to your secure Stripe billing portal...");
          window.location.href = data.url;
          return;
        } else {
          toast.error(
            "Stripe customer portal is not configured. Add STRIPE_SECRET_KEY in Settings.",
          );
        }
      } else {
        toast.error("Stripe customer portal is not configured. Add STRIPE_SECRET_KEY in Settings.");
      }
    } catch (err) {
      console.error(err);
...
================================================`;
    ...
    toast.success("Billing receipt downloaded successfully.");
  };
  ...
  const conciergeScore =
    tier === "Elite" ? "S+" : tier === "Premium" ? "S" : tier === "Standard" ? "A+" : "B";
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 sm:px-8 py-12 sm:py-16 space-y-8">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button
            onClick={() => {
              ...
              // also sign out of firebase if used
        ...
        <div className="space-y-6 animate-fade-up">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <div>
                ...
                <span className="font-display text-xl gold-text block mt-0.5">
                  {user?.user_metadata?.subscription_tier || "Standard"} Plan
                </span>
              </div>
              <div>
                <span className="text-xs uppercase tracking-widest text-muted-foreground block">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="glass rounded-xl p-6 space-y-4">
              ...
              <button
                ...
                className="w-full mt-2 border border-gold/30 text-gold py-2.5 rounded-md text-xs font-semibold hover:bg-gold/10 transition flex items-center justify-center gap-2 disabled:opacity-50 hover-lift"
              >
                {isOpeningPortal ? (
                  <>
                    <Loader2 className="h-3.5 w-3.5 animate-spin" /> Accessing Customer Portal...
                  </>
                ) : (
                  <>
                    <RefreshCw className="h-3 w-3" /> Manage Billing via Stripe Portal
                  </>
                )}
              </button>
            </div>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {[
              ...
              const currentTier = user?.user_metadata?.subscription_tier || "Standard";
      ...
      {/* ADMIN DASHBOARD TAB */}
      {tab === "admin" && !isAdmin && (
        <div className="glass rounded-xl p-10 text-center space-y-5 max-w-lg mx-auto my-12 border border-red-500/20 shadow-luxe animate-fade-up">
          <div className="grid h-12 w-12 place-items-center rounded-full bg-red-950/40 border border-red-500/30 text-red-400 mx-auto shadow-gold-glow">
            <Shield className="h-6 w-6" />
          </div>
          <div>
            <h2 className="font-display text-2xl text-red-400">Access Restricted</h2>
            <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
              Platinum Admin credentials are required to access the Atelier Control Center. Your
              current role does not have administrative authorization.
            </p>
          </div>
          <button
            onClick={() => setTab("overview")}
            className="gold-gradient text-primary-foreground px-6 py-2.5 rounded-md text-xs font-semibold hover-lift"
          >
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        <select
                          className="bg-muted text-foreground border border-gold/10 rounded px-2 py-1 text-[10px] focus:outline-none focus:border-gold"
                          defaultValue={p.subscription_tier || "Standard"}
                          onChange={async (e) => {
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                {profileAvatarUrl ? (
                  ...
                  <div className="w-12 h-12 rounded-full border border-dashed border-muted-foreground/40 flex items-center justify-center text-[10px] text-muted-foreground">
                    None
                  </div>
                )}
                <div className="flex flex-col gap-1 w-full">
                  ...
                  <button
                    ...
                    className="text-xs text-gold/80 hover:text-gold text-left underline underline-offset-2"
                  >
                    Create 3D Avatar via Ready Player Me
                  </button>
                </div>
              </div>
            </div>
